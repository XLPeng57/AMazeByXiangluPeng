...
public class BSPBuilder {
	 ...
	 */
	private BSPNode genNodes(MazePanel panel, ArrayList<Wall> walls) throws InterruptedException {
		// Recursion anchor:
		...
		splitWalls(panel, walls, splitter, left, right);
		...
		return new BSPBranch(splitter.getStartPositionX(), splitter.getStartPositionY(), 
				...
				genNodes(panel,left), genNodes(panel, right)); 
	}
	/**
	 ...
	 */
	private void splitWalls(MazePanel panel, ArrayList<Wall> walls, Wall splitter, final ArrayList<Wall> left, final ArrayList<Wall> right) {
		...
		for (int i = 0; i != walls.size(); i++) {
			...
			if (getSign(dot1) != getSign(dot2)) {
				if (dot1 == 0)
					...
					Wall[] sps = se.calculatePartitioning(panel, splitter, colchange);
					if (dot1 > 0) {
						right.add(sps[0]);
						left.add(sps[1]);
					} else {
	 ...
	 * @throws InterruptedException 
	 */
	public BSPNode generateBSPNodes(MazePanel panel) throws InterruptedException {
		...
		ArrayList<Wall> walls = builder.generateWalls(panel); 
