...
import edu.wm.cs.cs301.xianglupeng.gui.Constants;
...
public class ListOfWallsBuilder {
	 ...
	 */
	public ArrayList<Wall> generateWalls(MazePanel panel) {
		...
		if (false) {
		generateWallsForHorizontalWallboards(panel,sl); 

		generateWallsForVerticalWallboards(panel,sl);
		}
		else {
		    generateWallsForHorizontalWallboardsNew(panel, sl); 

	        generateWallsForVerticalWallboardsNew(panel, sl);
		}
     ...
     * @param sl
     */
	   private void generateWallsForVerticalWallboardsNew(MazePanel panel, ArrayList<Wall> sl) {
	        int x;
	        int y;
	        Iterator<int[]> it;
	        int[] cur;
	        ...
	        for (x = 0; x < width; x++) {
	            ...
	            while(it.hasNext()) {
                    ...
                    sl.add(new Wall(panel, x*Constants.MAP_UNIT, starty*Constants.MAP_UNIT,
                ...
                while(it.hasNext()) {
                    ...
                    sl.add(new Wall(panel, (x+1)*Constants.MAP_UNIT, y*Constants.MAP_UNIT,
     ...
     * @param sl
     */
    private void generateWallsForHorizontalWallboardsNew(MazePanel panel, ArrayList<Wall> sl) {
        int x;
        int y;
        Iterator<int[]> it;
        int[] cur;
        ...
        for (y = 0; y < height; y++) {
            ...
            while(it.hasNext()) {
                ...
                sl.add(new Wall(panel, x* Constants.MAP_UNIT, y*Constants.MAP_UNIT,
            ...
            while(it.hasNext()) {
                ...
                sl.add(new Wall(panel, startx*Constants.MAP_UNIT, (y+1)*Constants.MAP_UNIT,
	 ...
	 * @deprecated
	 */
	private void generateWallsForVerticalWallboards(MazePanel panel, ArrayList<Wall> sl) {
		int x;
		int y;
		// we search for vertical wallboards, so for each row
		for (x = 0; x != width; x++) {
	 ...
	 * @deprecated
	 */
	private void generateWallsForHorizontalWallboards(MazePanel panel, ArrayList<Wall> sl) {
		int x;
		int y;
		// we search for horizontal wallboards, so for each column
		for (y = 0; y != height; y++) {
