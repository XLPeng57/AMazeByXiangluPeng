...
import edu.wm.cs.cs301.xianglupeng.generation.BSPBranch;
import edu.wm.cs.cs301.xianglupeng.generation.BSPLeaf;
import edu.wm.cs.cs301.xianglupeng.generation.BSPNode;
import edu.wm.cs.cs301.xianglupeng.generation.Distance;
import edu.wm.cs.cs301.xianglupeng.generation.Floorplan;
import edu.wm.cs.cs301.xianglupeng.generation.Maze;
import edu.wm.cs.cs301.xianglupeng.generation.MazeContainer;
import edu.wm.cs.cs301.xianglupeng.generation.Wall;
...
public class MazeFileReader {
	 ...
	 */
	public MazeFileReader(MazePanel panel, String filename) {
		load(panel, filename) ;
	}
	 ...
	 */
	private void load(MazePanel panel, String filename)
	{
		try{
			...
			for (int temp = 0 ; temp < nList.getLength() ; temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					root = readBSPNode(panel, eElement);
				}
			}
		}
	 ...
	 */
	private BSPNode readBSPNode(MazePanel panel, Element eElement) {
		...
		if (isleafLoad)
		{
			...
			for (int i = 0 ; i < n ; i++)
			{
				slist.add(readWall(panel,eElement, number, i)) ;
			}
		...
		else
		{
			...
			number++ ;
			BSPNode l = readBSPNode(panel, eElement) ; // recursion updates index number for each element of the subtree
			number++ ; // increment index number for next node to visit
			BSPNode r = readBSPNode(panel, eElement) ;
	 ...
	 */
	private static Wall readWall(MazePanel panel, Element eElement, int number, int i) {
		...
		Wall result = new Wall(panel, x,y,dx,dy,dist,cc) ;
