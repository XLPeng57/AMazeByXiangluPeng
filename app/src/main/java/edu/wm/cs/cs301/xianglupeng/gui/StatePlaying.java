...
//    private CompassRose cr; // compass rose to show current direction
     ...
     * @param mazePanel is part of the UI and visible on the screen, needed for drawing
     */
...
//        		(int)(0.1*Constants.VIEW_HEIGHT),35);
        ...
        if (panel != null) {
        	startDrawer();
        	Log.v("StatePlaying: ", "start drawer.");
        }
        else {
...
//
//        	try {
//        		//((ReliableRobot)control.getRobot()).setController(controller);
...
//        }
	...
	protected void startDrawer() {
		...
		mapView = new Map(seenCells, 70, mazeConfig) ;
    ...
    public boolean keyDown(Constants.UserInput key, int value) {
        ...
        case Up: // move forward
            ...
            if (playManuallyActivity != null){
                walk(1);
            }else if (playAnimationActivity != null){
                if(robot.hasStopped()){
                    playAnimationActivity.tolose();
                }else{
                    walk(1);
                }
            }
            ...
            if (isOutside(px,py)) {
//                control.switchFromPlayingToWinning(0,0);
                if (playManuallyActivity == null){
                    playAnimationActivity.towin();
                }else{
                    playManuallyActivity.towin();
                }

                Log.v("StatePlaying: ", "outside the maze.");
            }

            // check termination, did we leave the maze?


//            if (playAnimationActivity != null){
//                if(robot.hasStopped()){
//                    playAnimationActivity.tolose();
//                }
//            }
